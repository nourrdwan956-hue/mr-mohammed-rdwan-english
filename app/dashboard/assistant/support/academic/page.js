...
export default function AssistantAcademicQuestionsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 6. جلب الأسئلة الأكاديمية عبر API الجديد
      const res = await fetch('/api/assistant/support?type=academic', {
        headers: { 'x-assistant-id': assistantData.id },
      });
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(errorData.error || 'فشل جلب الأسئلة');
      }
      const data = await res.json();
      if (!data.success) throw new Error('فشل جلب الأسئلة');
        ...
        {confirmModal && confirmModal.type === 'delete' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              ...
              <h3 className="text-lg font-bold mb-2">حذف السؤال</h3>
              <p className="text-sm mb-4">هل أنت متأكد من حذف هذا السؤال نهائياً؟</p>
