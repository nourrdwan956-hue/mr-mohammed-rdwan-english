// ============================================================
// app/dashboard/teacher/exams/[id]/results/page.js
// نتائج الامتحان – النسخة الأسطورية V13 (مع تباين عالٍ جداً وتكامل كامل)
...
// ✅ استيراد دوال التصحيح والأمان
import { gradeExam, calculateSecurityIndex, getExamStatistics } from '@/lib/examUtils';
...
// 3.2 صف الطالب في الجدول (مع تباين عالٍ) – معدل لإضافة الأعمدة الجديدة
const StudentRow = ({ 
  student, 
  index, 
  passingMarks, 
...
// 3.3 نافذة تقرير الطالب (مودال متقدم) – معدل لاستخدام gradeExam
...
// ============================================================
// 4. الصفحة الرئيسية – نتائج الامتحان (مع تباين عالٍ)
// ============================================================
export default function ExamResultsPage() {
  ...
  const fetchResults = useCallback(async () => {
    ...
    try {
      ...
      const { data: questionsData } = await supabase
        ...
        .select('*')
        .eq('exam_id', examId)
        .neq('type', 'passage') // ✅ استبعاد القطع
      ...
      const securityMap = {};
      processed.forEach(a => {
        // استخدام calculateSecurityIndex مع بيانات الطالب والأسئلة
  ...
  const chartData = useMemo(() => {
    ...
    // ✅ رسم بياني لمؤشرات الأمان (أعلى 10 طلاب)
  ...
  const handleSaveAttempts = async (studentId, newAttempts) => {
    try {
      // تحديث حقل custom_attempts_limit في جدول exam_attempts
  ...
  const goToQuestions = () => router.push(`/dashboard/teacher/exams/${examId}/questions`);
  ...
  return (
    <TeacherLayout>
      <div className={`relative ${isDark ? 'bg-[#0b0e1a]' : 'bg-gray-50'}`}>
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
            <div>
              <h1 className={`text-3xl font-extrabold ${isDark ? 'text-white' : 'text-gray-900'}`}>📊 نتائج الامتحان</h1>
              <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'} text-sm mt-1 flex items-center gap-2 flex-wrap`}>
                <span>{exam.title}</span>
                <span className="text-yellow-400">({stats.total} محاولة)</span>
                {course && (
                  <Link href={`/dashboard/teacher/courses/${course.id}`} className="text-xs text-blue-400 hover:text-blue-300 transition flex items-center gap-1">
            ...
            <div className="flex flex-wrap gap-3 mt-3 md:mt-0">
              ...
              <button
                onClick={goBack}
                className={`px-4 py-2 ${isDark ? 'bg-white/10 hover:bg-white/20 text-white border border-white/20' : 'bg-gray-200 hover:bg-gray-300 text-gray-800 border border-gray-300'} rounded-xl text-sm font-semibold transition flex items-center gap-2`}
              >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
            {statsData.map((stat) => <StatCard key={stat.id} stat={stat} styles={{ card: isDark ? 'bg-[#1a1f2e] border border-white/20' : 'bg-white border border-gray-300', cardBorder: isDark ? 'border-white/20' : 'border-gray-300', text: isDark ? 'text-white' : 'text-gray-900', subtext: isDark ? 'text-gray-400' : 'text-gray-600' }} />)}
          </div>

          {/* ===== الرسوم البيانية ===== */}
          {attempts.length > 0 && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
              <div className={`p-5 rounded-2xl ${isDark ? 'bg-[#1a1f2e] border border-white/20' : 'bg-white border border-gray-300 shadow-sm'}`}>
                <h3 className={`text-sm font-bold ${isDark ? 'text-white' : 'text-gray-900'} mb-4 text-center`}>توزيع درجات الطلاب</h3>
                <div className="h-56">
                  <Bar
              ...
              <div className={`p-5 rounded-2xl ${isDark ? 'bg-[#1a1f2e] border border-white/20' : 'bg-white border border-gray-300 shadow-sm'}`}>
                <h3 className={`text-sm font-bold ${isDark ? 'text-white' : 'text-gray-900'} mb-4 text-center`}>نسبة النجاح والرسوب</h3>
              ...
              {/* ✅ رسم بياني لمؤشرات الأمان */}
              <div className={`p-5 rounded-2xl ${isDark ? 'bg-[#1a1f2e] border border-white/20' : 'bg-white border border-gray-300 shadow-sm'}`}>
                <h3 className={`text-sm font-bold ${isDark ? 'text-white' : 'text-gray-900'} mb-4 text-center`}>🔒 مؤشرات الأمان (أعلى 10)</h3>
                <div className="h-56">
                  <Bar
          ...
          <div className={`p-5 rounded-2xl ${isDark ? 'bg-[#1a1f2e] border border-white/20' : 'bg-white border border-gray-300 shadow-sm'}`}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              <h3 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-gray-900'} flex items-center gap-2`}>
                ...
                <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>({filteredAttempts.length})</span>
              </h3>
              <div className="flex flex-wrap gap-2">
                <div className="relative flex-1 min-w-[150px]">
                  ...
                  <input
                    ...
                    className={`w-full p-2 pr-8 rounded-lg border focus:ring-2 focus:ring-yellow-500 outline-none transition text-sm ${isDark ? 'bg-[#0b0e1a] border-white/20 text-white placeholder-gray-400' : 'bg-white border-gray-300 text-gray-900 placeholder-gray-500'}`}
                  />
                </div>
                <select
                  value={filterStatus}
                  onChange={(e) => setFilterStatus(e.target.value)}
                  className={`p-2 rounded-lg border focus:ring-2 focus:ring-yellow-500 outline-none transition text-sm ${isDark ? 'bg-[#0b0e1a] border-white/20 text-white' : 'bg-white border-gray-300 text-gray-900'}`}
                >
                  ...
                  <tbody>
                    {filteredAttempts.map((attempt, index) => (
                      <StudentRow
                        ...
                        styles={{ text: isDark ? 'text-white' : 'text-gray-900', subtext: isDark ? 'text-gray-400' : 'text-gray-600', borderColor: isDark ? 'border-white/10' : 'border-gray-200' }}
                      />
                    ))}
                  </tbody>
                </table>
            ...
            <div className={`mt-4 pt-3 border-t ${isDark ? 'border-white/10' : 'border-gray-200'} flex justify-between text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          ...
          {/* ===== روابط سريعة ===== */}
          <div className={`p-4 mt-6 rounded-2xl ${isDark ? 'bg-[#1a1f2e] border border-white/20' : 'bg-white border border-gray-300 shadow-sm'}`}>
      ...
      <StudentReportModal
        ...
        exam={exam}
        questions={questions}
        styles={{ text: isDark ? 'text-white' : 'text-gray-900', subtext: isDark ? 'text-gray-400' : 'text-gray-600' }}
        theme={theme}
      />
