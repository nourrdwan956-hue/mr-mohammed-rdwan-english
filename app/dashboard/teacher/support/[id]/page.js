...
// ================================================================
// 💬 المسار: app/dashboard/teacher/support/[id]/page.js
// صفحة تفاصيل الدعم – المحادثة الكاملة مع إجراءات المعلم المتكاملة
// ================================================================
// الميزات:
// - عرض تفاصيل الطلب (شكوى/سؤال) مع معلومات الطالب والكورس والتصنيف.
// - سلسلة ردود بتصميم فقاعات محادثة (المعلم يمين، الطالب يسار).
...
// - حظر / فك حظر الطالب من نفس الصفحة.
// - حذف التذكرة بالكامل.
...
// ----- useTheme محلي -----
const useTheme = () => {
  const [theme, setTheme] = useState(() => { try { return localStorage.getItem('teacherTicketTheme') || 'dark'; } catch { return 'dark'; } });
  useEffect(() => { localStorage.setItem('teacherTicketTheme', theme); document.documentElement.className = theme; }, [theme]);
...
export default function TeacherSupportDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (ticketData.assigned_to !== u.id) {
        ...
        router.push('/dashboard/teacher/support');
        return;
      }

      // التحقق من الحظر
      const { data: banData } = await supabase.from('support_bans')
        .select('*')
        .eq('teacher_id', u.id)
        .eq('student_id', ticketData.student_id)
        .is('unbanned_at', null)
        .maybeSingle();

      setTicket({ ...ticketData, is_banned: !!banData });
  ...
  const handleSendReply = async (e) => {
    ...
    try {
      ...
      // تحديث first_reply_at إذا كان أول رد للمعلم
  ...
  const handleToggleBan = async () => {
    ...
    if (ticket.is_banned) {
      const { error } = await supabase.from('support_bans').update({ unbanned_at: new Date().toISOString() }).eq('teacher_id', user.id).eq('student_id', studentId).is('unbanned_at', null);
      if (error) { toast.error('فشل فك الحظر'); return; }
      setTicket(prev => ({ ...prev, is_banned: false }));
      toast.success('تم فك الحظر');
    } else {
      const { error } = await supabase.from('support_bans').insert({ teacher_id: user.id, student_id: studentId });
      if (error) { toast.error('فشل الحظر'); return; }
      setTicket(prev => ({ ...prev, is_banned: true }));
    ...
    <TeacherLayout>
      <div className={`min-h-screen ${styles.bg} ${styles.text} flex flex-col`} dir="rtl">
        ...
        <div className={`sticky top-0 z-20 ${styles.card} border-b ${styles.border} backdrop-blur-md px-4 py-3 flex items-center gap-3`}>
          <button onClick={() => router.push('/dashboard/teacher/support')} className={`p-2 rounded-xl hover:bg-white/5 ${styles.subtext}`}><Icons.ArrowRight className="h-5 w-5" /></button>
        ...
        <div className="flex-1 max-w-7xl mx-auto w-full p-4 md:p-6 grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className={`${styles.card} border ${styles.border} rounded-2xl p-5 mb-4`}>
              ...
              <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
                {replies.length === 0 ? (
                  ...
                  replies.map(reply => {
                    const isTeacher = reply.sender_id === user?.id;
                    return (
                      <motion.div key={reply.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                        className={`flex ${isTeacher ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[85%] rounded-2xl p-4 ${isTeacher ? 'bg-gradient-to-br from-yellow-400/20 to-yellow-600/20 border border-[var(--border-color)]' : `${styles.card} border ${styles.border}`}`}>
                          {!isTeacher && <p className="text-[10px] font-semibold text-blue-400 mb-1">{reply.sender?.full_name || 'طالب'}</p>}
          ...
          <div className="lg:col-span-1 space-y-4">
            <div className={`${styles.card} border ${styles.border} rounded-2xl p-4`}>
              ...
              {/* حظر / فك حظر (خاص بالمعلم) */}
              {!isAssistant && (
                <button onClick={handleToggleBan}
                  className={`w-full p-2 rounded-lg text-sm mb-2 ${ticket.is_banned ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                  {ticket.is_banned ? 'فك الحظر' : 'حظر الطالب'}
                </button>
              )}
            ...
            <div className={`${styles.card} border ${styles.border} rounded-2xl p-4`}>
              ...
              <Link href={`/dashboard/teacher/messages/${ticket.student_id}`} className="flex items-center gap-2 p-2 rounded-lg hover:bg-white/5 text-sm mb-1">
                <Icons.Mail className="h-4 w-4 text-blue-400" /> مراسلة الطالب
              </Link>
              <Link href={`/dashboard/teacher/notes/${ticket.student_id}`} className="flex items-center gap-2 p-2 rounded-lg hover:bg-white/5 text-sm mb-1">
                <Icons.StickyNote className="h-4 w-4 text-purple-400" /> إضافة ملاحظة
              </Link>
              <Link href={`/dashboard/teacher/announcements/new?student=${ticket.student_id}`} className="flex items-center gap-2 p-2 rounded-lg hover:bg-white/5 text-sm">
