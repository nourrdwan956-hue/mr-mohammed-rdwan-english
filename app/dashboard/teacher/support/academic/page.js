...
// ================================================================
// 🎓 المسار: app/dashboard/teacher/support/academic/page.js
// قائمة الأسئلة الأكاديمية للمعلم – النسخة المتطورة V4.0
// ================================================================
// الميزات:
// - عرض جميع الأسئلة الأكاديمية مع فلترة متقدمة (حالة، أولوية، كورس، تصنيف، بحث).
// - إحصائيات حية: عدد الأسئلة المفتوحة، المعلقة، المحلولة.
// - إجراءات سريعة لكل سؤال: تغيير الحالة، رد، حظر/فك حظر، حذف.
...
// ----- useTheme محلي -----
const useTheme = () => {
  const [theme, setTheme] = useState(() => { try { return localStorage.getItem('teacherAcadTheme') || 'dark'; } catch { return 'dark'; } });
  useEffect(() => { localStorage.setItem('teacherAcadTheme', theme); document.documentElement.className = theme; }, [theme]);
...
const CATEGORY_MAP = {
  grammar: { label: 'قواعد', color: 'text-violet-400', icon: Icons.BookOpen },
  vocabulary: { label: 'مفردات', color: 'text-green-400', icon: Icons.Book },
  reading: { label: 'قراءة', color: 'text-blue-400', icon: Icons.Eye },
  writing: { label: 'كتابة', color: 'text-orange-400', icon: Icons.Edit },
  listening: { label: 'استماع', color: 'text-pink-400', icon: Icons.Headphones },
  speaking: { label: 'تحدث', color: 'text-cyan-400', icon: Icons.Mic },
  exam_prep: { label: 'تحضير امتحان', color: 'text-yellow-400', icon: Icons.FileText },
...
export default function AcademicQuestionsPage() {
  ...
  const handleToggleBan = async (question) => {
    const studentId = question.student_id;
    const isBanned = question.is_banned;
    if (isBanned) {
  ...
  const filteredQuestions = useMemo(() => {
    ...
    if (filterPriority !== 'all') result = result.filter(q => q.priority === filterPriority);
    if (filterCourse !== 'all') result = result.filter(q => q.course_id === filterCourse);
    if (filterCategory !== 'all') result = result.filter(q => q.question_category === filterCategory);
      ...
      <div className={`min-h-screen ${styles.bg} ${styles.text} p-4 md:p-6`}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl md:text-4xl font-extrabold bg-gradient-to-r from-blue-400 to-indigo-500 bg-clip-text text-transparent">
                🎓 الأسئلة الأكاديمية
              </h1>
              <p className={`${styles.subtext} text-sm mt-1`}>إدارة الأسئلة الدراسية حول منهج اللغة الإنجليزية</p>
            </div>
          ...
          <div className="flex flex-col md:flex-row gap-3 mb-6 flex-wrap">
            ...
            <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)} className={`p-2.5 ${styles.input} border ${styles.border} rounded-xl`}>
              <option value="all">جميع التصنيفات</option>
              {Object.entries(CATEGORY_MAP).map(([key, val]) => <option key={key} value={key}>{val.label}</option>)}
            </select>
          ...
          {filteredQuestions.length === 0 ? (
            ...
            <div className="space-y-3">
              {filteredQuestions.map(question => {
                const statusInfo = STATUS_MAP[question.status];
                const priorityInfo = PRIORITY_MAP[question.priority];
                const categoryInfo = CATEGORY_MAP[question.question_category] || CATEGORY_MAP.other;
                ...
                return (
                  <motion.div key={question.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} whileHover={{ y: -2 }}
                    ...
                    onClick={() => router.push(`/dashboard/teacher/support/${question.id}`)}>
                    <div className="flex flex-col md:flex-row md:items-center gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          ...
                          {question.unit && <span className="text-[10px] bg-blue-500/10 text-blue-400 px-2 py-0.5 rounded-full">📘 {question.unit}</span>}
                          {question.is_banned && <span className="text-[10px] bg-red-500/10 text-red-400 px-2 py-0.5 rounded-full">محظور</span>}
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0" onClick={e => e.stopPropagation()}>
                        ...
                        {!isAssistant && (
                          <button onClick={() => confirmAction(question.is_banned ? 'unban' : 'ban', question.id, question.subject)}
                            className={`text-xs px-3 py-1.5 rounded-lg ${question.is_banned ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                            {question.is_banned ? 'فك الحظر' : 'حظر'}
                          </button>
                        )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              ...
              <h3 className="text-lg font-bold mb-2">
                {confirmModal.type === 'delete' ? 'حذف السؤال' : confirmModal.type === 'ban' ? 'حظر الطالب' : 'فك الحظر'}
              </h3>
              <p className="text-sm mb-4">{confirmModal.type === 'delete' ? 'هل أنت متأكد من حذف هذا السؤال نهائياً؟' : confirmModal.type === 'ban' ? 'سيتم منع الطالب من إرسال أسئلة وشكاوى جديدة.' : 'سيتم السماح للطالب بإرسال الأسئلة والشكاوى مرة أخرى.'}</p>
              <div className="flex gap-3">
                <button onClick={() => {
                  if (confirmModal.type === 'delete') handleDelete(confirmModal.id);
                  else if (confirmModal.type === 'ban') handleToggleBan(questions.find(q => q.id === confirmModal.id));
                  else if (confirmModal.type === 'unban') handleToggleBan(questions.find(q => q.id === confirmModal.id));
