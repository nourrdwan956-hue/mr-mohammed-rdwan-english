...
// ============================================================
// استيراد أسئلة من ملف – النسخة النهائية (مع إصلاح toast)
// ============================================================
    ...
    return (
      <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--primary-color)', borderRadius: '12px', padding: '16px', marginBottom: '12px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          {/* حقل الوسوم مع اقتراحات من البنك */}
          <div>
...
export default function ImportQuestionsPage() {
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--bg-card', isDark ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.9)');
  ...
  // استخلاص الأسئلة عبر API (مع تحسين الأخطاء)
  const handleExtract = async () => {
    ...
    try {
      ...
      if (data.questions.length === 0) {
        ...
        // تطبيق الإعدادات الافتراضية على الأسئلة المستخلصة
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      if (importMode === 'replace') {
        // حذف جميع الأسئلة الحالية في البنك
        ...
        // جلب الأسئلة الحالية لتجنب المكررات (بناءً على النص)
      ...
      // في وضع append لا نقوم بأي معالجة إضافية
      ...
      setFilePreview('');
      setTimeout(() => {
        router.push(`/dashboard/teacher/question-bank/${bankId}`);
      }, 1500);
    } catch (err) {
      console.error(err);
      ...
      <div style={{ backgroundColor: 'var(--bg-primary)', color: 'var(--text-primary)', minHeight: '100vh', transition: 'all 0.3s' }}>
        <div style={{ maxWidth: '1024px', margin: '0 auto', padding: '20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
            <Link href={`/dashboard/teacher/question-bank/${bankId}`} style={{ color: 'var(--text-muted)', transition: 'color 0.2s' }}>
