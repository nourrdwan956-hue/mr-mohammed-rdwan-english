// ============================================================
// app/dashboard/teacher/students/[id]/page.js
// تفاصيل الطالب – الملف الشخصي المتكامل V9
// ============================================================
...
import { useTheme } from '@/lib/hooks/useTheme'; // ✅ جلب اللغة
...
const CourseProgressCard = ({ course, progress, completedAt }) => {
  ...
  return (
    <motion.div
      whileHover={{ y: -4 }}
      className="bg-white/5 border border-white/10 rounded-xl p-4 hover:border-yellow-400/30 transition-all duration-300"
    >
...
const ExamResultItem = ({ attempt, exam }) => {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          <h4 className="text-white font-medium">{exam?.title || 'امتحان'}</h4>
          <p className="text-xs text-gray-400 mt-1">{formatDate(attempt.completed_at)}</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <Link
            href={`/dashboard/teacher/exams/${attempt.exam_id}/results`}
            className="p-1.5 hover:bg-blue-500/20 rounded-lg transition text-blue-400 hover:text-blue-300"
          >
            <Icons.Eye className="h-4 w-4" />
          </Link>
...
const VideoWatchItem = ({ video, progress }) => {
...
// ============================================================
// 6. الصفحة الرئيسية – تفاصيل الطالب
// ============================================================
...
export default function TeacherStudentDetailPage() {
  ...
  const { language } = useTheme(); // ✅ جلب اللغة
  ...
  const fetchStudentData = useCallback(async () => {
    ...
    try {
      ...
      // 2. جلب تسجيلات الطالب في الكورسات
    ...
    return (
      <TeacherLayout>
        <div className="text-center py-20">
          ...
          <p className="text-red-400 text-lg">الطالب غير موجود</p>
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-xs text-gray-400">#{studentId.slice(0, 8)}</span>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              {/* ✅ زر واتساب للطالب - مع تنظيف الرقم وإضافة رمز البلد */}
              {profile?.phone && (
                <button
                  onClick={() => {
              ...
              {/* ✅ زر واتساب لولي الأمر - مع تنظيف الرقم وإضافة رمز البلد */}
              ...
              <button
                onClick={goBack}
                className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-xl text-xs hover:border-yellow-400/50 transition flex items-center gap-1"
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            <div className="lg:col-span-1">
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden hover:border-yellow-400/30 transition-all duration-500">
                <div className="p-6 text-center">
                  {/* ✅ صورة رمزية مع دعم الصورة الرمزية */}
                  ...
                  <h2 className="text-2xl font-bold text-white mt-4">{profile.full_name || 'طالب'}</h2>
                  <p className="text-sm text-gray-400">{profile.email}</p>
                  {profile.phone && (
                    <p className="text-sm text-gray-400 mt-1 dir-ltr">{profile.phone}</p>
                  )}
                  {/* ✅ عرض اسم ولي الأمر (اختياري) */}
                  {profile.parent_name && (
                ...
                <div className="border-t border-white/5 p-4">
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div className="text-center">
                      ...
                      <p className="text-white font-medium">{profile.role === 'student' ? 'طالب' : profile.role}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-gray-400">انضم</p>
                    ...
                    {profile.school && (
                      <div className="text-center">
                        <p className="text-gray-400">المدرسة</p>
                        <p className="text-white font-medium">{profile.school}</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              {enrollments.length > 0 && (
                <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-5">
                  <h3 className="text-sm font-bold text-white mb-4 text-center">توزيع تقدم الطالب في الكورسات</h3>
                  <div className="h-56">
                    <Bar
              ...
              {examAttempts.length > 0 && (
                <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-5">
                  <h3 className="text-sm font-bold text-white mb-4 text-center">توزيع نتائج الامتحانات</h3>
                  <div className="h-56">
                    <Bar
          ...
          {/* ===== التبويبات ===== */}
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-5 hover:border-yellow-400/30 transition-all duration-500">
                ...
                >
                  {enrollments.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-400">لم يسجل الطالب في أي كورس بعد</p>
                    </div>
                  ) : (
                ...
                >
                  {examAttempts.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-400">لم يقم الطالب بأي امتحان بعد</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                ...
                >
                  {watchHistory.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-400">لم يشاهد الطالب أي فيديو بعد</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
          ...
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-4 mt-6">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link href="/dashboard/teacher/reports" className="text-xs bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded-lg transition text-gray-300 hover:text-white">التقارير</Link>
            </div>
          </div>
        </div>
      </div>
