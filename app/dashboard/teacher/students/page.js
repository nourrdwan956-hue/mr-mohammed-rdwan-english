...
// إدارة الطلاب – النسخة الأسطورية المتكاملة V8
// ============================================================
...
const StudentCard = ({
  ...
  return (
    <motion.div
      ...
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      className="group relative bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden hover:border-yellow-400/50 transition-all duration-500 hover:shadow-2xl hover:shadow-yellow-400/10"
    >
      ...
      <div className="relative z-10 p-5">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex items-center md:items-start gap-4 md:gap-3 flex-1 min-w-0">
            <div className={`h-16 w-16 rounded-full bg-gradient-to-br ${avatarColor} flex items-center justify-center text-white font-extrabold text-2xl shadow-lg flex-shrink-0`}>
              {initials}
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-bold text-white group-hover:text-yellow-300 transition-colors cursor-pointer" onClick={() => onViewProfile(student.id)}>
                  {student.name}
                </h3>
              ...
              <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-gray-400">
                <span className="flex items-center gap-1.5 bg-white/5 px-2 py-0.5 rounded-full">
                  <Icons.Book className="h-3 w-3 text-yellow-400" />
                  {student.coursesCount} كورس
                </span>
                <span className="flex items-center gap-1.5 bg-white/5 px-2 py-0.5 rounded-full">
                  <Icons.FileText className="h-3 w-3 text-purple-400" />
                  {student.examsCount} امتحان
                </span>
                <span className="flex items-center gap-1.5 bg-white/5 px-2 py-0.5 rounded-full">
                  <Icons.Calendar className="h-3 w-3" />
                  {student.lastActive ? new Date(student.lastActive).toLocaleDateString('ar-EG') : 'غير نشط'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6 mt-3 md:mt-0">
            <div className="flex items-center gap-4">
              <div className="text-center">
                <p className="text-2xl font-extrabold text-yellow-400">{Math.round(student.avgProgress)}%</p>
                <p className="text-[10px] text-gray-400">التقدم</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-extrabold text-green-400">{student.avgScore || 0}</p>
            ...
            <div className="flex flex-wrap gap-2 flex-shrink-0">
              ...
              >
                <Icons.Eye className="h-3 w-3" /> ملف
              </button>
              <button
              ...
              <button
                onClick={() => onUnenroll(student.id, student.name)}
        ...
        {student.courses && student.courses.length > 0 && (
          <div className="mt-4 pt-3 border-t border-white/5 flex flex-wrap gap-1.5">
            {student.courses.slice(0, 4).map((course, i) => (
              <span key={i} className="text-[10px] bg-white/5 px-2 py-0.5 rounded-full text-gray-400 border border-white/5">
                {course}
              </span>
            ))}
...
// ============================================================
// 6. نافذة تأكيد إلغاء التسجيل
// ============================================================

const UnenrollModal = ({ isOpen, onClose, onConfirm, studentName }) => {
    ...
    >
      ...
      >
        <div className="text-center">
          ...
          <h3 className="text-xl font-bold text-white mb-2">تأكيد إلغاء التسجيل</h3>
          <p className="text-gray-400 text-sm mb-6">
            هل أنت متأكد من إلغاء تسجيل "{studentName}" من جميع كورساتك؟
          </p>
...
// ============================================================
// 7. الصفحة الرئيسية – إدارة الطلاب
// ============================================================
...
export default function TeacherStudentsPage() {
      ...
      enrollmentsData?.forEach(en => {
        ...
        if (!studentMap[studentId]) {
          studentMap[studentId] = {
            id: studentId,
            name: en.profiles?.full_name || 'طالب',
            email: en.profiles?.email || '',
            phone: en.profiles?.phone || '',
            courses: [],
            avgProgress: 0,
            completedAt: null,
            lastActive: null,
        ...
        s.coursesCount += 1;
        if (en.completed_at) {
          if (!s.completedAt || new Date(en.completed_at) > new Date(s.completedAt)) {
            s.completedAt = en.completed_at;
          }
        }
        if (!s.lastActive || new Date(en.updated_at) > new Date(s.lastActive)) {
          s.lastActive = en.updated_at;
        }
      });
  ...
  const filteredStudents = useMemo(() => {
    ...
    switch (sortBy) {
      case 'progress':
        result.sort((a, b) => (b.avgProgress || 0) - (a.avgProgress || 0));
        break;
      case 'name':
  ...
  const handleUnenrollClick = (studentId, studentName) => {
    setUnenrollTarget({ id: studentId, name: studentName });
  ...
  const confirmUnenroll = async () => {
    ...
    try {
      // جلب جميع تسجيلات هذا الطالب في كورسات المعلم
      ...
      if (courseIds.length > 0) {
        const { error } = await supabase
          .from('enrollments')
          .delete()
          .eq('student_id', unenrollTarget.id)
          .in('course_id', courseIds);

        if (error) throw error;
      }

      toast.success(`✅ تم إلغاء تسجيل ${unenrollTarget.name} بنجاح`);
  ...
  return (
    <TeacherLayout>
      <div className="relative">
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl font-extrabold text-white">👥 إدارة الطلاب</h1>
          ...
          <div className="flex flex-col md:flex-row gap-3 mb-6">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="ابحث عن طالب (اسم أو بريد)..."
            ...
            >
              ...
              <option value="score">المتوسط</option>
            </select>
          </div>
            ...
            <div className="text-center py-20 bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl">
              ...
              <p className="text-gray-400 text-sm mt-2">
                {searchQuery || filterStatus !== 'all' || filterCourse !== 'all'
                  ...
                  : 'سيظهر الطلاب هنا عند تسجيلهم في كورساتك'}
              </p>
            </div>
          ) : (
      ...
      {/* ===== نافذة تأكيد إلغاء التسجيل ===== */}
      <UnenrollModal
        isOpen={isUnenrollModalOpen}
        onClose={() => setIsUnenrollModalOpen(false)}
        onConfirm={confirmUnenroll}
        studentName={unenrollTarget?.name}
      />

      {/* ===== روابط سريعة ===== */}
      <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-4 mt-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Link href="/dashboard/teacher/videos" className="text-xs bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded-lg transition text-gray-300 hover:text-white">الفيديوهات</Link>
