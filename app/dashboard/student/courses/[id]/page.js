...
// ================================================================
// مكونات فرعية مكبرة وواضحة
// ================================================================

const TabButton = ({ active, onClick, icon: Icon, label, count, styles }) => (
  <button
    onClick={onClick}
    className={`flex items-center gap-3 px-6 py-4 rounded-xl text-lg font-bold transition-all duration-300 ${
  ...
  >
    ...
    <span>{label}</span>
    {count !== undefined && (
      <span className={`text-sm rounded-full px-3 py-0.5 ${active ? 'bg-blue-500/30 text-blue-700 dark:text-blue-300' : 'bg-gray-200 dark:bg-white/10'}`}>{count}</span>
    )}
  </button>
);
...
const CircularProgress = ({ percentage, size = 80, strokeWidth = 6, label, styles }) => (
  <div className="relative inline-flex items-center justify-center">
    <svg width={size} height={size} className="transform -rotate-90">
      <circle cx={size/2} cy={size/2} r={size/2 - strokeWidth/2} className="stroke-current text-gray-200 dark:text-white/10" strokeWidth={strokeWidth} fill="none" />
      <motion.circle cx={size/2} cy={size/2} r={size/2 - strokeWidth/2} stroke="url(#grad)" strokeWidth={strokeWidth} fill="none" strokeLinecap="round"
        strokeDasharray={(size/2 - strokeWidth/2) * 2 * Math.PI}
        initial={{ strokeDashoffset: (size/2 - strokeWidth/2) * 2 * Math.PI }}
        animate={{ strokeDashoffset: (size/2 - strokeWidth/2) * 2 * Math.PI * (1 - percentage/100) }}
    ...
    <div className="absolute flex flex-col items-center">
      <span className={`text-xl font-extrabold ${styles.text}`}>{Math.round(percentage)}%</span>
      {label && <span className={`text-sm ${styles.subtext} -mt-0.5`}>{label}</span>}
    </div>
  </div>
);
...
const VideoItem = ({ video, bookmarked, onToggleBookmark, styles, language, watched }) => {
  ...
  return (
    <WaveBorderCard initialColor={color.name} onColorChange={handleColorChange}>
      <div className={`p-5 flex items-center gap-5 hover:border-${color.name}-400/50 transition group relative min-h-[80px]`}>
        {watched && (
          <div className="absolute top-3 right-3 bg-green-500/20 text-green-400 rounded-full p-1.5">
            <Icons.CheckCircle className="h-5 w-5 fill-current" />
          </div>
        )}
        <div className="relative flex-shrink-0">
          <div className={`w-16 h-16 rounded-xl ${watched ? 'bg-green-400/10' : 'bg-blue-400/10'} flex items-center justify-center`}>
            <Icons.Play className={`h-8 w-8 ${watched ? 'text-green-500' : `text-${color.name}-500`}`} />
          </div>
          {video.duration && (
            <span className="absolute -bottom-1 -right-1 bg-black/80 text-white text-xs px-2 py-0.5 rounded-md font-mono">
              {video.duration}
            </span>
          )}
        </div>
        <div className="flex-1 min-w-0">
          <Link href={`/watch/${video.id}`} className={`text-lg font-bold ${styles.text} hover:text-${color.name}-500 transition line-clamp-1`}>
            {video.title}
          </Link>
          {video.description && <p className={`text-sm ${styles.subtext} line-clamp-2 mt-1`}>{video.description}</p>}
        </div>
        <button onClick={() => onToggleBookmark(video.id)} className={`p-3 rounded-xl transition ${bookmarked ? `text-${color.name}-500 bg-${color.name}-400/10` : 'text-gray-400 hover:text-yellow-500 hover:bg-yellow-400/5'}`}>
          <Icons.Bookmark className={`h-6 w-6 ${bookmarked ? 'fill-current' : ''}`} />
        </button>
      </div>
    </WaveBorderCard>
  );
...
const ExamItem = ({ exam, styles, language, attempted, score }) => {
  ...
  return (
    <WaveBorderCard initialColor={color.name} onColorChange={handleColorChange}>
      <div className={`p-5 flex items-center gap-5 hover:border-${color.name}-400/50 transition min-h-[80px]`}>
        <div className={`w-16 h-16 rounded-xl ${attempted ? 'bg-blue-400/10' : 'bg-emerald-400/10'} flex items-center justify-center flex-shrink-0`}>
          <Icons.FileText className={`h-8 w-8 ${attempted ? 'text-blue-500' : 'text-emerald-500'}`} />
        </div>
        <div className="flex-1">
          <Link href={`/dashboard/student/exams/${exam.id}`} className={`text-lg font-bold ${styles.text} hover:text-${color.name}-500 transition`}>{exam.title}</Link>
          {exam.duration_minutes && <p className={`text-sm ${styles.subtext}`}>{exam.duration_minutes} {language==='ar'?'دقيقة':'min'}</p>}
          {attempted && score !== undefined && (
            <div className="flex items-center gap-3 mt-1">
              <span className={`text-base font-bold ${score >= (exam.passing_marks || 50) ? 'text-green-400' : 'text-red-400'}`}>
...
const BookItem = ({ book, styles, language }) => {
  ...
  return (
    <WaveBorderCard initialColor={color.name} onColorChange={handleColorChange}>
      <div className={`p-5 flex items-center gap-5 hover:border-${color.name}-400/50 transition min-h-[80px]`}>
        <div className="w-16 h-16 rounded-xl bg-purple-400/10 flex items-center justify-center flex-shrink-0">
          <Icons.BookOpen className="h-8 w-8 text-purple-500" />
        </div>
        <div className="flex-1">
          <Link href={`/dashboard/student/books/${book.id}`} className={`text-lg font-bold ${styles.text} hover:text-${color.name}-500 transition`}>{book.title}</Link>
        </div>
      </div>
    </WaveBorderCard>
  );
...
// ================================================================
// صفحة تفاصيل الكورس – نسخة معدلة (إزالة تقييم، مشاركة، مراجعات)
// ================================================================
      ...
      <div className={`min-h-screen flex items-center justify-center ${styles.bg} p-4`}>
        <div className={`max-w-md w-full p-8 rounded-3xl ${styles.card} border ${styles.border} text-center shadow-2xl`}>
          <div className="inline-flex p-4 rounded-full bg-red-500/20 border-2 border-red-500/30">
            <Icons.Lock className="h-16 w-16 text-red-400" />
          </div>
          <h2 className={`text-2xl font-extrabold ${styles.text} mt-4`}>
            {language === 'ar' ? '🚫 وصول ممنوع' : '🚫 Access Denied'}
          </h2>
          <p className={`text-base ${styles.subtext} mt-2 leading-relaxed`}>{message}</p>
  ...
  return (
    <div className={`w-full min-h-screen ${styles.bg}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-10">
        {/* ===== هيدر الكورس مع Wave Border ===== */}
        <WaveBorderCard initialColor={headerColor.name} onColorChange={setHeaderColor}>
          <div className="p-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-1">
                <div className="aspect-video rounded-2xl overflow-hidden bg-gradient-to-br from-gray-800/50 to-gray-900/50 border border-[var(--border-color)] relative shadow-2xl">
                  {course.cover_image ? (
                    <img
                  ...
                  {enrolled && (
                    <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-md rounded-xl px-4 py-2 text-sm font-bold text-white flex items-center gap-3">
                      <div className={`w-3 h-3 rounded-full ${progress === 100 ? 'bg-green-400' : 'bg-yellow-400 animate-pulse'}`} />
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="flex flex-wrap items-center gap-3">
                  <span className={`px-4 py-2 rounded-full text-sm font-bold ${course.is_free||course.price===0?'bg-green-500/20 text-green-400':'bg-blue-500/20 text-blue-400'} backdrop-blur-sm border border-current/20`}>
                    {course.is_free||course.price===0? (language==='ar'?'مجاني':'Free') : `${course.price} جنيه`}
                  </span>
                  <span className="px-4 py-2 rounded-full text-sm font-bold bg-purple-500/10 text-purple-400 border border-purple-400/20">
                ...
                <h1 className={`text-4xl sm:text-5xl font-extrabold ${styles.text} leading-tight`}>{course.title}</h1>

                {teacher && (
                  <div className="flex items-center gap-4 p-4 rounded-xl bg-gradient-to-r from-blue-500/5 to-transparent border border-blue-400/10">
                    ...
                    <div>
                      <p className={`text-xl font-bold ${styles.text}`}>{teacher.full_name}</p>
                      <p className={`text-sm ${styles.subtext}`}>{language==='ar'?'المعلم':'Teacher'}{teacher.email ? ` • ${teacher.email}` : ''}</p>
                    </div>
                  </div>
                )}

                {/* ❌ تم حذف StarRating بالكامل */}

                {course.description && (
                  <div className={`p-5 rounded-xl ${styles.card} border ${styles.border}`}>
                    <h4 className={`text-lg font-bold ${styles.text} mb-3`}>{language==='ar'?'وصف الكورس':'Description'}</h4>
                    <p className={`text-base ${styles.subtext} leading-relaxed`}>{course.description}</p>
                  </div>
                )}
                ...
                {/* لوحة تحكم مصغرة مكبرة */}
                {enrolled && (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className={`p-5 rounded-xl ${styles.card} border ${styles.border} text-center`}>
                      <CircularProgress percentage={videosProgress} size={80} strokeWidth={6} styles={styles} />
                      <p className={`text-sm mt-3 ${styles.subtext}`}>{language==='ar'?'الفيديوهات':'Videos'} ({completedVideos}/{totalVideos})</p>
                    </div>
                    <div className={`p-5 rounded-xl ${styles.card} border ${styles.border} text-center`}>
                      <CircularProgress percentage={examsProgress} size={80} strokeWidth={6} styles={styles} />
                      <p className={`text-sm mt-3 ${styles.subtext}`}>{language==='ar'?'الامتحانات':'Exams'} ({attemptedExams}/{totalExams})</p>
                    </div>
                    <div className={`p-5 rounded-xl ${styles.card} border ${styles.border} text-center`}>
                      <CircularProgress percentage={progress} size={80} strokeWidth={6} styles={styles} />
                      <p className={`text-sm mt-3 ${styles.subtext}`}>{language==='ar'?'التقدم العام':'Overall'}</p>
                    </div>
                    {lastWatched && (
                      <div className={`p-5 rounded-xl ${styles.card} border ${styles.border} flex flex-col justify-center`}>
                        <p className={`text-sm ${styles.subtext}`}>{language==='ar'?'آخر مشاهدة':'Last Watched'}</p>
                        <Link href={`/watch/${lastWatched.video_id}`} className={`text-base font-bold text-blue-500 hover:underline line-clamp-2 mt-2`}>{lastWatched.video?.title || 'فيديو'}</Link>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="flex flex-wrap gap-4 pt-3">
                  {enrolled ? (
                    <>
                      <Link href={`/dashboard/student/courses/${id}/progress`} className="px-8 py-4 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 text-white font-bold text-lg hover:scale-105 transition shadow-2xl shadow-blue-500/30 flex items-center gap-2">
                    ...
                    <div className="w-full text-center py-8">
                      <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        {course.is_free || course.price === 0 ? (
                          <button onClick={handleEnroll} disabled={enrolling} className="px-10 py-4 bg-gradient-to-r from-green-500 to-green-600 text-white font-bold rounded-2xl hover:scale-105 transition shadow-2xl shadow-green-500/30 text-xl">
                            {enrolling ? (language==='ar'?'جاري...':'Loading...') : (language==='ar'?'ابدأ الآن 🚀':'Start Now 🚀')}
                          </button>
                        ) : (
                          <>
                            <button onClick={() => router.push(`/dashboard/student/courses/${id}/payment`)} className="px-10 py-4 bg-gradient-to-r from-blue-500 to-blue-600 text-white font-bold rounded-2xl hover:scale-105 transition shadow-2xl shadow-blue-500/30 text-xl">
                              {language==='ar'?'💳 الاشتراك الآن':'💳 Subscribe Now'}
                            </button>
                            <button onClick={handleEnroll} disabled={enrolling} className="px-6 py-4 bg-gradient-to-r from-gray-500/20 to-gray-600/20 text-gray-400 font-bold rounded-2xl hover:scale-105 transition border border-gray-500/30 text-lg">
        ...
        {enrolled && (
          <>
            <div className="flex gap-3 border-b-2 border-gray-200 dark:border-white/10 pb-3 overflow-x-auto">
              ...
              <TabButton 
                ...
                onClick={() => {
                  // توجيه إلى صفحة السؤال الأكاديمي مع تمرير course_id
                  router.push(`/dashboard/student/support/academic?course=${id}`);
                }} 
                icon={Icons.MessageCircle} 
                label={language==='ar'?'إرسال سؤال أكاديمي':'Send Academic Question'} 
                styles={styles}
              />
            </div>
            <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} className="space-y-4">
              {contentLoading ? (
                <div className="flex justify-center py-20"><div className="w-16 h-16 border-4 border-blue-500/20 border-t-blue-500 rounded-full animate-spin"/></div>
              ) : (
                <>
                  {activeTab==='videos' && (
                    <div className="space-y-4">
                      ...
                      )) : <p className={`text-lg ${styles.subtext}`}>{language==='ar'?'لا توجد فيديوهات':'No videos yet'}</p>}
                    </div>
                  )}
                  {activeTab==='exams' && (
                    <div className="space-y-4">
                      ...
                      )) : <p className={`text-lg ${styles.subtext}`}>{language==='ar'?'لا توجد امتحانات':'No exams yet'}</p>}
                    </div>
                  )}
                  {activeTab==='books' && (
                    <div className="space-y-4">
                      ...
                      )) : <p className={`text-lg ${styles.subtext}`}>{language==='ar'?'لا توجد كتب':'No books yet'}</p>}
                    </div>
                  )}
                  {/* ❌ تم حذف تبويب المراجعات بالكامل */}
                </>
              )}
            </motion.div>
          </>
        ...
        {!enrolled && (
          // ===== عرض القفل للكورسات غير المشترك فيها (مع دعم الكورسات المدفوعة) =====
          <div className="text-center py-28 border-2 border-dashed border-gray-300 dark:border-white/10 rounded-3xl">
            <Icons.Lock className="h-24 w-24 text-gray-400 mx-auto mb-5"/>
            <h3 className={`text-3xl font-bold ${styles.text} mb-4`}>
            ...
            <p className={`text-lg ${styles.subtext} max-w-lg mx-auto`}>
              {course.is_free
            ...
            {course.is_free ? (
              <button onClick={handleEnroll} disabled={enrolling} className="mt-6 px-10 py-4 bg-gradient-to-r from-green-500 to-green-600 text-white font-bold rounded-2xl hover:scale-105 transition shadow-2xl shadow-green-500/30 text-xl">
                {enrolling ? (language==='ar'?'جاري...':'Loading...') : (language==='ar'?'ابدأ الآن 🚀':'Start Now 🚀')}
              </button>
            ) : (
        ...
        {relatedCourses.length>0 && (
          <div>
            <h2 className={`text-2xl font-bold ${styles.text} mb-5 flex items-center gap-3`}>
              <Icons.Grid3X3 className="h-7 w-7 text-blue-500"/> {language==='ar'?'كورسات ذات صلة':'Related Courses'}
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {relatedCourses.map(rc=>(
                <Link key={rc.id} href={`/dashboard/student/courses/${rc.id}`} className={`p-6 rounded-xl border ${styles.border} ${styles.card} hover:border-blue-400/50 transition group`}>
                  <div className="flex items-center gap-4 mb-3">
                    <div className="w-14 h-14 rounded-xl bg-blue-500/10 flex items-center justify-center"><Icons.BookOpen className="h-7 w-7 text-blue-500 group-hover:scale-110 transition"/></div>
                    <span className={`text-lg font-bold ${styles.text} line-clamp-1`}>{rc.title}</span>
                  </div>
                  {rc.teacher && <p className={`text-sm ${styles.subtext}`}>{rc.teacher.full_name}</p>}
                </Link>
              ))}
            </div>
          </div>
