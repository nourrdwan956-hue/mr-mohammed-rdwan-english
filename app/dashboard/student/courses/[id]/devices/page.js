        ...
        <div className="p-5 flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className={`text-base font-bold ${styles.text}`}>{deviceName}</h3>
              {device.is_primary && (
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className={`p-5 rounded-2xl ${styles.card} border ${styles.border} text-center`}>
            ...
            <p className={`text-sm ${styles.subtext}`}>{language === 'ar' ? 'الأجهزة المسجلة' : 'Registered Devices'}</p>
          </div>
          <div className={`p-5 rounded-2xl ${styles.card} border ${styles.border} text-center`}>
            <Icons.CheckCircle className="h-8 w-8 text-emerald-500 mx-auto mb-2" />
            <p className={`text-3xl font-bold ${styles.text}`}>{stats.maxDevices}</p>
            <p className={`text-sm ${styles.subtext}`}>{language === 'ar' ? 'الحد الأقصى' : 'Maximum Devices'}</p>
          </div>
          <div className={`p-5 rounded-2xl ${styles.card} border ${styles.border} text-center`}>
            <Icons.AlertCircle className={`h-8 w-8 ${stats.isFull ? 'text-red-500' : 'text-yellow-500'} mx-auto mb-2`} />
            <p className={`text-3xl font-bold ${stats.isFull ? 'text-red-400' : 'text-yellow-400'}`}>
