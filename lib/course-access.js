 ...
 * - الجهاز الأول يُسجل كأساسي (is_primary = true).
 * - إذا تم حذف جهاز (is_active = false) يمكن إعادة تنشيطه.
 * @param {string} studentId - معرف الطالب
 * @param {string} courseId - معرف الكورس
...
export async function registerDeviceIfNeeded(studentId, courseId, fingerprint = null) {
  try {
    ...
    if (!fp) fp = `fallback-${studentId}-${courseId}`;
    ...
    // 2. التحقق من وجود الجهاز (سواء نشط أو غير نشط)
    const { data: existingDevice, error: existingError } = await supabase
      .from('course_devices')
      .select('*')
      .eq('student_id', studentId)
    ...
    // إذا كان الجهاز موجوداً ولكن غير نشط، نعيد تنشيطه (بدلاً من إنشاء جديد)
    if (existingDevice) {
      if (!existingDevice.is_active) {
        ...
        if (reactivateError) {
          console.error('❌ Error reactivating device:', reactivateError);
          return { success: false, message: 'فشل إعادة تنشيط الجهاز' };
        }
        console.log('✅ Device reactivated');
        return { success: true, device: existingDevice, message: 'تم إعادة تنشيط الجهاز' };
      } else {
        // الجهاز نشط بالفعل → تحديث وقت الاستخدام
        await supabase
          .from('course_devices')
          .update({ last_used_at: new Date().toISOString() })
          .eq('id', existingDevice.id);
    ...
    // 3. الجهاز غير موجود → نعد الأجهزة النشطة
    const { count, error: countError } = await supabase
      .from('course_devices')
      .select('id', { count: 'exact', head: true })
      .eq('student_id', studentId)
    ...
    if (currentDevices >= maxDevices) {
      console.warn('⚠️ Max devices reached');
      return { success: false, message: 'تم تجاوز الحد الأقصى للأجهزة' };
    }

    // 4. تسجيل جهاز جديد (الأول يصبح أساسي)
    const deviceName = getDeviceName();
    const deviceInfo = getDeviceInfo();
    ...
    const { data: newDevice, error: insertError } = await supabase
      ...
      .insert({
        ...
        device_info: deviceInfo,
        is_active: true,
        is_primary: currentDevices === 0, // الأول = أساسي
        first_used_at: new Date().toISOString(),
        last_used_at: new Date().toISOString(),
      })
      .select()
